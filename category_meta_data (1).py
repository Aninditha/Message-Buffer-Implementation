...
def cat_meta_data_write(ob):
        ...
        if ob.category=='general':
            ...
            f.write(creat+'|')
            time=struct.pack("20s",ob.timestamp)
            f.write(time+"|")
...
def pool_seek_add_forum(cat_pool,forum_count,forum_name):
    ...
    meta_present=meta_start+(14*forum_count)

    f.seek(meta_present)
    id=struct.pack("10s",forum_name)
    f.write(id)
    f.seek(0,1)
    n=f.read(4)
...
def pool_seek_read_forum(cat_pool,forum_count):
    ...
    meta_present=meta_start+(14*forum_count)
    f.seek(meta_present)
    forum_name=f.read(10)
    id,=struct.unpack("10s",forum_name)
    f.seek(0,1)
    n=f.read(4)
    forum_addr,=struct.unpack('i',n)
    return  id,addr
...
def retrieve_forum_addr(cat_pool,forum_count,forum_name):
    ...
    i=0
    forum_addr= -1
    forum_name=struct.pack("10s",forum_name)
    while i<forum_count:
        meta_present=meta_start+(14*i)
        f.seek(meta_present)
        id=f.read(10)
        id,=struct.unpack("10s",id)
        f.seek(0,1)
        n=f.read(4)
...
if __name__=="__main__":
    ...
    ob.category='politics'
    ob.forum_name="iran war"
    ob.createdby="us"
    ob.timestamp='1234567890123456'
    for i in range(20):
      insert_forum(ob)
    q=ques_create()
    q.question="where is iran"
    q.answer="It is near saudi"
    q.question_id=1
